 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'x_magazine_About' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class x_magazine_About {
		/**
		 ...
		 *
		 * @var x_magazine_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof x_magazine_About ) ) {
				self::$instance = new x_magazine_About;
    ...
    return add_query_arg(
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://wpthemespace.com/product/x-magazine/').'" target="_blank" >';
					echo esc_html ('X Magazine Pro Live Preview', 'x-magazine');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_stylesheet_directory_uri() . '/inc/about/css/about.min.css', array(),'2.3.5','all'  );
        ...
        'activating_string'        => __( 'Activating', 'x-magazine' )
        )
    );
